...
本项目是蘑菇科普智能服务平台的后端服务，围绕食用菌领域，提供从 **图像识别** → **毒性判断** → **AI 科普问答** → **种植管理** 的完整知识服务闭环。系统分为用户端和管理端两大模块，支持多角色权限控制。
...
| Redis | 6.0+ |
| MinIO | 最新版 |
| Python | 3.8+（蘑菇识别推理脚本需要） |
| Maven | 3.8+ |

### 1. 克隆项目

```bash
git clone https://github.com/your-username/mh-web-server.git
cd mh-web-server
```

### 2. 配置文件

将 `application-example.yml` 复制为 `application.yml`，并按需修改其中的占位符：
...
- **MinIO** — 对象存储端点与密钥
- **邮件服务** — SMTP 邮箱账号与授权码
- **火山引擎 Ark** — 大模型 API Key 与模型端点 ID
- **AI 识别** — Python 环境路径与 ONNX 模型路径

### 3. 初始化数据库

创建 MySQL 数据库 `mh_mushroom`，并导入项目中的 SQL 脚本建表。表名统一使用 `tb_` 前缀。

### 4. 启动 MinIO

确保 MinIO 服务已运行，并创建名为 `mushroom` 的 Bucket。
